    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      <div v-for="member in members" 
        ...
        <div class="p-6 flex flex-col flex-grow">
          <div>
            <h3 class="text-xl font-semibold text-invoke-text group-hover:text-invoke-accent transition-colors duration-300">{{ member.name }}</h3>
          ...
          <div class="mt-4 flex-grow">
            ...
            <button v-if="member.hasOverflow"
                    @click="toggleBio(member)"
          ...
          <div class="mt-4 flex flex-wrap gap-2">
            <span v-for="skill in member.skills" 
                  :key="skill"
                  class="px-2 py-1 text-xs bg-invoke-accent/10 rounded-full text-invoke-text/80 group-hover:bg-invoke-accent/20 transition-colors duration-300">
              {{ skill }}
            </span>
          </div>
